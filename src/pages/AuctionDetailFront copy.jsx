...
import { FiClock, FiDollarSign, FiUsers, FiHeart, FiShare2, FiCheckCircle, FiAlertCircle, FiCreditCard, FiMapPin, FiCalendar, FiEye } from 'react-icons/fi'
...
const AuctionDetailFront = () => {
  ...
  if (error) return (
    <div className="min-vh-100 d-flex align-items-center justify-content-center">
      <div className="text-center">
        <h3>Error</h3>
        <p>{error}</p>
        <button className="btn btn-primary" onClick={() => navigate('/auctions')}>← Back to Auctions</button>
      </div>
    </div>
  )
  ...
  return (
    <>
      ...
      {/* Premium Header */}
      <header className="bg-white shadow-sm border-bottom sticky-top" style={{ zIndex: 1000 }}>
                ...
                <ol className="breadcrumb mb-0" style={{ backgroundColor: 'transparent' }}>
                  <li className="breadcrumb-item">
                    <a href="/" className="text-decoration-none text-dark fw-medium">
                      <HiHome className="me-2" size={18} style={{ color: '#6c757d' }} />
                      Home
                    </a>
                  </li>
                  <li className="breadcrumb-item">
                    <HiChevronRight className="mx-2" size={14} style={{ color: '#dee2e6' }} />
                    <a href="/auctions" className="text-decoration-none text-dark fw-medium">
                      <MdGavel className="me-2" size={18} style={{ color: '#6c757d' }} />
                      Live Auctions
                    </a>
                  </li>
                  <li className="breadcrumb-item active">
                    <HiChevronRight className="mx-2" size={14} style={{ color: '#dee2e6' }} />
                    <span className="text-primary fw-semibold">{auction.carListing?.name}</span>
                  </li>
                </ol>
              </nav>
            </div>
            <div className="col-md-6 text-end">
              <span className={`badge ${statusBadge.class} me-3 px-3 py-2 fs-6 fw-semibold`} style={{ borderRadius: '25px' }}>
                {statusBadge.text}
              </span>
              <span className={`badge ${typeBadge.class} px-3 py-2 fs-6 fw-semibold`} style={{ borderRadius: '25px' }}>
                {typeBadge.text}
              </span>
            </div>
          </div>
      ...
      <div className="container-fluid py-5">
        <div className="row g-4">
          ...
          <div className="col-lg-8">
            ...
            <div className="bg-white rounded-4 shadow-sm p-4 mb-4 border-0">
              <div className="d-flex justify-content-between align-items-start mb-3">
                <div>
                  <h1 className="display-6 fw-bold text-dark mb-2" style={{ lineHeight: '1.2' }}>
                  ...
                  <div className="d-flex align-items-center gap-4 text-secondary">
                    <div className="d-flex align-items-center">
                      ...
                      <span className="fw-medium">{auction.carListing?.year}</span>
                    </div>
                    <div className="d-flex align-items-center">
                      <FiMapPin size={16} className="me-2 text-primary" />
                      <span className="fw-medium">{auction.carListing?.location}</span>
                    </div>
                {auction.carListing?.mileage && (
                      <div className="d-flex align-items-center">
                        <span className="me-2 text-primary">🏃</span>
                        <span className="fw-medium">{auction.carListing.mileage.toLocaleString()} miles</span>
                      </div>
                    )}
                    <div className="d-flex align-items-center">
                      <FiEye size={16} className="me-2 text-primary" />
                      <span className="fw-medium">{auction.viewCount || 0} views</span>
                    </div>
                  </div>
                </div>
                <div className="text-end">
                  <div className="badge bg-gradient bg-primary text-white px-3 py-2 rounded-pill fs-6">
                    🔥 Live Auction
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-4 shadow-sm mb-4 border-0 overflow-hidden">
              <div className="position-relative" style={{ height: '500px' }}>
                ...
                <div className="position-absolute top-0 start-0 p-4">
                  <div className="d-flex gap-2">
                    ...
                    {isActive && (
                      <span className="badge bg-danger text-white px-3 py-2 rounded-pill fw-semibold shadow-sm animate__animated animate__pulse animate__infinite">
                ...
                {auction.carListing?.media?.length > 1 && (
                  <>
                    <button
                      ...
                      onClick={() => setActiveIdx(activeIdx > 0 ? activeIdx - 1 : auction.carListing.media.length - 1)}
                    >
                    ...
                    <button
                      ...
                      style={{ width: '50px', height: '50px', opacity: 0.9 }}
                      onClick={() => setActiveIdx(activeIdx < auction.carListing.media.length - 1 ? activeIdx + 1 : 0)}
                    >
              ...
              {/* Premium Thumbnails */}
                ...
                <div className="p-4 bg-light bg-opacity-50">
                  <div className="d-flex gap-3 overflow-auto" style={{ scrollbarWidth: 'thin' }}>
                      {auction.carListing.media.map((m, i) => {
                        const thumbUrl = toAbsUrl(m.url)
                        return (
                          <button
                            key={i}
                          className={`btn p-0 border-0 position-relative overflow-hidden ${activeIdx === i ? 'ring ring-primary' : ''}`}
                          ...
                          >
                            {m.type === 'video' ? (
                            <video src={thumbUrl} className="w-100 h-100" style={{ objectFit: 'cover', borderRadius: '12px' }} />
                          ) : (
                            <img src={thumbUrl} alt={`${auction.carListing?.name}-${i}`} className="w-100 h-100" style={{ objectFit: 'cover', borderRadius: '12px' }} />
                          )}
                          {activeIdx === i && (
                            <div className="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" style={{ backgroundColor: 'rgba(0,123,255,0.2)' }}>
                              <i className="fas fa-play-circle text-primary" style={{ fontSize: '24px' }}></i>
                            </div>
                            )}
                          </button>
                        )
              ...
              <div className="p-4">
                <div className="d-flex align-items-center mb-4">
                  <div className="bg-primary bg-opacity-10 rounded-circle p-3 me-3">
                    <i className="fas fa-car text-primary" style={{ fontSize: '24px' }}></i>
              </div>
                  <div>
                    <h3 className="fw-bold text-dark mb-1">Vehicle Specifications</h3>
                    <p className="text-secondary mb-0">Detailed information about this vehicle</p>
                  </div>
                </div>
                  ...
                  {auction.carListing?.mileage && (
                    <div className="col-md-6">
                      <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                        ...
                        <div>
                          <div className="fw-bold text-dark">{auction.carListing.mileage.toLocaleString()} miles</div>
                  ...
                  {auction.carListing?.condition && (
                    <div className="col-md-6">
                      <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                        ...
                        <div>
                          <div className="fw-bold text-dark">{auction.carListing.condition}</div>
                  ...
                  {auction.carListing?.color && (
                    <div className="col-md-6">
                      <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                        ...
                        <div>
                          <div className="fw-bold text-dark">{auction.carListing.color}</div>
                  ...
                  {auction.carListing?.transmission && (
                    <div className="col-md-6">
                      <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                        ...
                        <div>
                          <div className="fw-bold text-dark">{auction.carListing.transmission}</div>
                          <small className="text-secondary">Transmission</small>
                        </div>
                      </div>
                    </div>
                  )}
                  {auction.carListing?.fuelType && (
                    <div className="col-md-6">
                      <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                        ...
                        <div>
                          <div className="fw-bold text-dark">{auction.carListing.fuelType}</div>
                  ...
                  {auction.carListing?.drivetrain && (
                    <div className="col-md-6">
                      <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                        ...
                        <div>
                          <div className="fw-bold text-dark">{auction.carListing.drivetrain}</div>
                  ...
                  <div className="mt-5">
                    ...
                    <div className="p-4 bg-light bg-opacity-30 rounded-3">
                      <div 
                        ...
                        style={{ fontSize: '16px' }}
                        dangerouslySetInnerHTML={{ __html: auction.carListing.descriptionHtml }} 
                      />
                    </div>
                  </div>
                )}
            ...
            <div className="bg-white rounded-4 shadow-sm border-0">
              <div className="p-4">
                <div className="d-flex align-items-center mb-4">
                  ...
                  <div>
                    <h3 className="fw-bold text-dark mb-1">Auction Details</h3>
                    <p className="text-secondary mb-0">Complete auction information and timeline</p>
                  </div>
                </div>
                ...
                <div className="row g-4">
                  <div className="col-md-6">
                    <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                      ...
                      <div>
                        <div className="fw-bold text-dark">{typeBadge.text}</div>
                  ...
                  <div className="col-md-6">
                    <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                      ...
                      <div>
                        <div className="fw-bold text-dark">${auction.startingBid?.toLocaleString() || '0'}</div>
                  ...
                  <div className="col-md-6">
                    <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                      ...
                      <div>
                        <div className="fw-bold text-dark">${auction.bidIncrement?.toLocaleString() || '100'}</div>
                        <small className="text-secondary">Bid Increment</small>
                      </div>
                    </div>
                  </div>
                  <div className="col-md-6">
                    <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                      ...
                      <div>
                        <div className="fw-bold text-dark">{new Date(auction.startDate).toLocaleDateString()}</div>
                  ...
                  <div className="col-md-6">
                    <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                      ...
                      <div>
                        <div className="fw-bold text-dark">{new Date(auction.endDate).toLocaleDateString()}</div>
                  ...
                  {auction.reservePrice && (
                    <div className="col-md-6">
                      <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                        ...
                        <div>
                          <div className="fw-bold text-dark">${auction.reservePrice.toLocaleString()}</div>
                          <small className="text-secondary">Reserve Price</small>
                        </div>
                      </div>
                    </div>
                  )}
          ...
          <div className="col-lg-4">
            ...
            <div className="bg-white rounded-4 shadow-lg mb-4 border-0 position-sticky" style={{ top: '100px' }}>
              <div className="p-4 text-center">
                <div className="bg-primary bg-opacity-10 rounded-circle d-inline-flex align-items-center justify-content-center mb-3" style={{ width: '60px', height: '60px' }}>
                  <MdGavel className="text-primary" size={28} />
                </div>
                <h6 className="text-secondary mb-2 fw-semibold">CURRENT HIGH BID</h6>
                <h1 className="display-4 fw-bold text-dark mb-2" style={{ fontFamily: 'monospace' }}>
                ...
                {isActive && (
                  <div className="p-3 bg-gradient bg-warning bg-opacity-10 rounded-3 border border-warning border-opacity-25">
                    <div className="d-flex align-items-center justify-content-center gap-2">
                      <div className="bg-warning bg-opacity-20 rounded-circle p-2">
                        <FiClock className="text-warning" size={20} />
                      </div>
                      <div>
                        <div className="fw-bold text-dark fs-5">{timeRemaining}</div>
                        <small className="text-secondary">Time Remaining</small>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="p-4 text-center">
                  {auction.reserveMet ? (
                    <div>
                      ...
                      <h5 className="fw-bold text-success mb-2">Reserve Met! 🎉</h5>
                      <p className="text-secondary mb-0">The reserve price has been reached. This auction will sell to the highest bidder.</p>
                    </div>
                  ) : (
                    <div>
                      <div className="bg-warning bg-opacity-10 rounded-circle d-inline-flex align-items-center justify-content-center mb-3" style={{ width: '50px', height: '50px' }}>
                        <FiAlertCircle className="text-warning" size={24} />
                      </div>
                      <h5 className="fw-bold text-warning mb-2">Reserve Not Met</h5>
                      <p className="text-secondary mb-0">Bidding has not yet reached the seller's reserve price.</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {canBuyNow && (
              <div className="bg-gradient-to-r from-success to-success bg-white rounded-4 shadow-lg mb-4 border-2 border-success border-opacity-25">
                <div className="p-4 text-center">
                  ...
                  <h5 className="fw-semibold text-dark mb-2">Skip the Wait - Buy It Now!</h5>
                  <h2 className="display-6 fw-bold text-success mb-3" style={{ fontFamily: 'monospace' }}>
                  ...
                  <p className="text-secondary mt-2 mb-0 small">Instant purchase - no waiting required</p>
                </div>
              </div>
            )}
            ...
            {canBid && (
              <div className="bg-white rounded-4 shadow-lg mb-4 border-0">
                <div className="p-4">
                  ...
                  <div className="mb-4">
                    <label className="form-label fw-semibold text-dark">Bid Amount</label>
                    <div className="input-group input-group-lg">
                      <span className="input-group-text bg-light border-end-0 fw-bold text-dark">$</span>
                    <input
                      type="number"
                        className="form-control border-start-0 fs-4 fw-bold text-center"
                      value={bidAmount}
                      onChange={(e) => setBidAmount(e.target.value)}
                        placeholder={(auction.currentBid > 0 ? auction.currentBid + (auction.bidIncrement || 100) : auction.startingBid).toLocaleString()}
                        style={{ 
                    ...
                    <div className="d-flex justify-content-between align-items-center mt-2">
                      <small className="text-secondary">
                        ...
                        Min: ${(auction.currentBid > 0 ? auction.currentBid + (auction.bidIncrement || 100) : auction.startingBid).toLocaleString()}
                      </small>
                      <small className="text-secondary">
                        Increment: ${auction.bidIncrement?.toLocaleString() || '100'}
                    </small>
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                      ...
                      <div className="flex-grow-1">
                        <label className="form-check-label fw-semibold text-dark d-block" htmlFor="proxyBidding">
                          🤖 Enable Auto-Bidding
                      </label>
                        <small className="text-secondary">Automatically bid up to your maximum amount</small>
                      </div>
                    </div>
                    {showProxyBid && (
                      <div className="mt-3">
                        <label className="form-label fw-semibold text-dark">Maximum Bid Amount</label>
                        <div className="input-group">
                          <span className="input-group-text bg-light fw-bold text-dark">$</span>
                      <input
                        type="number"
                        ...
                        <small className="text-info">
                          ...
                          We'll bid incrementally up to this amount to keep you winning
                        </small>
                      </div>
                    )}
                  </div>
                  ...
                  <button 
                    ...
                    style={{ 
                      padding: '14px 24px', 
                      fontWeight: '600',
                      background: bidding ? '#6c757d' : 'linear-gradient(135deg, #0d6efd 0%, #0b5ed7 100%)'
                    }}
                  >
                    {bidding ? (
                      <>
              ...
              <div className="bg-white rounded-4 shadow-lg mb-4 border-0">
                <div className="p-4 text-center">
                  <div className="bg-info bg-opacity-10 rounded-circle d-inline-flex align-items-center justify-content-center mb-3" style={{ width: '60px', height: '60px' }}>
                    <i className="fas fa-user-plus text-info" style={{ fontSize: '24px' }}></i>
                  </div>
                  <h4 className="fw-bold text-dark mb-2">Ready to Start Bidding? 🚀</h4>
                  <p className="text-secondary mb-4">Join thousands of bidders and start winning amazing deals!</p>
            ...
            <div className="bg-white rounded-4 shadow-sm mb-4 border-0">
              <div className="p-4">
                <div className="d-flex align-items-center mb-3">
                  ...
                  <h5 className="fw-bold text-dark mb-0">Seller Information</h5>
                </div>
                
                <div className="d-flex align-items-center p-3 bg-light bg-opacity-50 rounded-3">
                  <div style={{ width: '50px', height: '50px' }} className="me-3">
                    <img 
                      src={auction.seller?.avatarFile ? toAbsUrl(auction.seller.avatarFile) : '/assets/images/user-img-1.webp'} 
                      alt="seller" 
                  ...
                  <div className="flex-grow-1">
                    <h6 className="fw-bold text-dark mb-1">{auction.seller?.name || 'Unknown Seller'}</h6>
                    <div className="d-flex align-items-center gap-2">
                      <span className="badge bg-primary bg-opacity-20 text-primary px-2 py-1 rounded-pill small">
                        {auction.seller?.role === 'dealer' ? '🏢 Dealer' : '👤 Private Seller'}
                      </span>
                      <span className="badge bg-success bg-opacity-20 text-success px-2 py-1 rounded-pill small">
                        ✅ Verified
                      </span>
                    </div>
                  </div>
              ...
              <div className="p-4">
                ...
                {bids.length === 0 ? (
                  <div className="text-center py-5">
                    <div className="bg-light bg-opacity-50 rounded-circle d-inline-flex align-items-center justify-content-center mb-3" style={{ width: '60px', height: '60px' }}>
                      <i className="fas fa-gavel text-secondary" style={{ fontSize: '24px' }}></i>
                    </div>
                    <h6 className="text-secondary mb-2">No bids yet</h6>
                    <p className="text-secondary mb-0 small">Be the first to place a bid on this auction!</p>
                  </div>
                ) : (
                  <div style={{ maxHeight: '300px', overflowY: 'auto' }} className="bid-history">
                    {bids.slice(0, 10).map((bid, index) => (
                      <div key={bid._id || index} className={`d-flex align-items-center p-3 rounded-3 mb-2 ${bid.status === 'winning' ? 'bg-success bg-opacity-10 border border-success border-opacity-25' : 'bg-light bg-opacity-30'}`}>
                        <div className="me-3">
                          <div className={`rounded-circle d-flex align-items-center justify-content-center ${bid.status === 'winning' ? 'bg-success text-white' : 'bg-secondary text-white'}`} style={{ width: '35px', height: '35px' }}>
                            <i className={`fas ${bid.bidType === 'proxy' ? 'fa-robot' : 'fa-user'}`} style={{ fontSize: '14px' }}></i>
                        </div>
                        </div>
                        <div className="flex-grow-1">
                          <div className="d-flex align-items-center gap-2 mb-1">
                            <span className="fw-bold text-dark fs-5" style={{ fontFamily: 'monospace' }}>
                              ${(bid.amount || bid.bidAmount)?.toLocaleString()}
                            </span>
                            {bid.bidType === 'proxy' && (
                              <span className="badge bg-info bg-opacity-20 text-info px-2 py-1 rounded-pill" style={{ fontSize: '10px' }}>
                                🤖 AUTO
                              </span>
                            )}
                            {bid.status === 'winning' && (
                              <span className="badge bg-success text-white px-2 py-1 rounded-pill" style={{ fontSize: '10px' }}>
                                👑 WINNING
                              </span>
                            )}
                          </div>
                          <div className="d-flex align-items-center justify-content-between">
                            <small className="text-secondary fw-medium">{bid.bidder?.name || 'Anonymous Bidder'}</small>
                            <small className="text-secondary">
                ...
                <div className="d-flex align-items-center justify-content-between mb-4">
                  <div className="d-flex align-items-center">
                    <div className="bg-success bg-opacity-10 rounded-circle p-2 me-3">
                      ...
                      <small className="text-secondary">Real-time auction discussion</small>
                        </div>
                  </div>
                  {watchers > 1 && (
                    <div className="d-flex align-items-center gap-2">
                      <div className="bg-success bg-opacity-20 rounded-circle" style={{ width: '8px', height: '8px' }}></div>
                      <small className="text-success fw-semibold">{watchers} online</small>
                    </div>
                  )}
                </div>
                ...
                >
                  {loadingComments ? (
                    <div className="text-center py-5">
                      ...
                      <p className="text-secondary mb-0">Loading chat messages...</p>
                    </div>
                  ) : comments.length === 0 ? (
                    <div className="text-center py-5">
                      <div className="bg-light bg-opacity-50 rounded-circle d-inline-flex align-items-center justify-content-center mb-3" style={{ width: '60px', height: '60px' }}>
                        <i className="fas fa-comment-dots text-secondary" style={{ fontSize: '24px' }}></i>
                      </div>
                      <h6 className="text-secondary mb-2">No messages yet</h6>
                      <p className="text-secondary mb-0 small">Start the conversation and connect with other bidders!</p>
                    </div>
                  ) : (
                          ...
                          <div className={`p-3 rounded-3 shadow-sm ${
                            ...
                            <div className="d-flex align-items-center gap-2 mb-2">
                              {comment.type === 'comment' && (
                                <img 
                                  src={comment.user?.avatarFile ? toAbsUrl(comment.user.avatarFile) : '/assets/images/user-img-1.webp'} 
                              ...
                              <div className="flex-grow-1">
                                <div className="d-flex align-items-center justify-content-between">
                                  <span className="fw-bold" style={{ fontSize: '14px' }}>
                                    {comment.type === 'system' ? '🔔 System Alert' : 
                                     comment.type === 'bid_notification' ? '💰 Bid Update' : 
                                     comment.user?.name || 'Anonymous'}
                                  </span>
                                  <small className={comment.type === 'bid_notification' ? 'text-white-50' : 'text-secondary'}>
                    ...
                    <div className="typing-indicator p-3 bg-white bg-opacity-80 rounded-3 mb-3 border border-primary border-opacity-25">
                      <div className="d-flex align-items-center gap-2">
                        <div className="typing-animation d-flex gap-1">
                          <div className="bg-primary rounded-circle" style={{ width: '6px', height: '6px', animation: 'typing 1.4s infinite ease-in-out' }}></div>
                          <div className="bg-primary rounded-circle" style={{ width: '6px', height: '6px', animation: 'typing 1.4s infinite ease-in-out 0.2s' }}></div>
                          <div className="bg-primary rounded-circle" style={{ width: '6px', height: '6px', animation: 'typing 1.4s infinite ease-in-out 0.4s' }}></div>
              </div>
                  ...
                  {currentUser ? (
                    <div className="d-flex gap-3 align-items-end">
                      <div style={{ width: '40px', height: '40px' }}>
                        <img 
                          src={currentUser.avatarFile ? toAbsUrl(currentUser.avatarFile) : '/assets/images/user-img-1.webp'} 
                    ...
                    <div className="text-center p-4 bg-light bg-opacity-50 rounded-3">
                      <i className="fas fa-lock text-secondary mb-2" style={{ fontSize: '24px' }}></i>
                      <p className="text-secondary mb-2">Join the conversation</p>
                      <a href="/signin" className="btn btn-primary btn-sm rounded-pill px-4">
                        Sign in to chat
                      </a>
                    </div>
                  )}
